            ...
            if (endpoint.getParameters() != null && !endpoint.getParameters().trim().isEmpty() && !endpoint.getParameters().equals("[]")) {
                ...
                if (paramsNode.isArray()) {
                    for (com.fasterxml.jackson.databind.JsonNode param : paramsNode) {
                        ...
                        String desc = param.has("description") && !param.get("description").asText().trim().isEmpty() 
                            ? param.get("description").asText().trim() 
                            : getAutoGeneratedDescription(name, type);
...
""";
    ...
    private String getAutoGeneratedDescription(String name, String type) {
        if (name == null || name.trim().isEmpty()) return "Query parameter filter";
        String cleanName = name.toLowerCase().replaceAll("^[:_]+", "");
        if (cleanName.equals("id") || cleanName.endsWith("_id")) {
            String target = cleanName.equals("id") ? "record" : cleanName.replace("_id", "").replace("_", " ");
            return "Unique identifier filter for target " + target;
        }
        if (cleanName.equals("search") || cleanName.equals("q") || cleanName.equals("keyword") || cleanName.contains("search")) {
            return "Search query keyword to filter matching records";
        }
        if (cleanName.equals("status") || cleanName.endsWith("_status")) {
            return "Status state filter parameter";
        }
        if (cleanName.contains("date") || cleanName.contains("time") || cleanName.contains("created") || cleanName.contains("updated")) {
            return "Date filter constraint parameter (" + (type != null ? type : "string") + ")";
        }
        if (cleanName.equals("limit") || cleanName.equals("size")) {
            return "Maximum number of records to return per request";
        }
        if (cleanName.equals("offset") || cleanName.equals("page")) {
            return "Pagination offset or page number for result set";
        }
        return "Filter parameter for :" + name + " (" + (type != null ? type : "string") + ")";
    }
}
